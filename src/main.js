...
document.addEventListener("DOMContentLoaded", function () {
  ...
  var sibionicsReader = new FileReader();

  var activityStartTime = 0;
  var activityEndTime = 0;

  let fitData = null;
  let sibionicsData = null;
    ...
    if (fieldx === "timestamp" && fieldy.slice(0, 3) === "lap") {
      for (let k = 0; k < fitdatalocal.laps.length; k++) {
        dataPoints.push({
          ...
          y:
            fieldy === "lap_avg_heart_rate"
              ? fitdatalocal.laps[k].avg_heart_rate
              : fitdatalocal.laps[k].total_elapsed_time,
          //y: k,
  ...
  document.getElementById("ylist").onchange = function (e) {
    ...
    if (
      (yobj.value.slice(0, 1) === "v" && xobj.value === "time_hrv") ||
  ...
  var span = (document.getElementsByClassName("close")[0].onclick =
    function () {
      modal.style.display = "none";
    });
  // popup windows to change line properties: color, thickness
  ...
  function updateMapSegment(e) {
    ...
    const showMapSegmentFlag = !(
      document.getElementById("xaxis").value === "lap_number" ||
        ...
        for (let k in fitdata.records) {
          ...
          if (record[xname] >= xMin && record[xname] <= xMax) {
            ...
            for (let n = 0; n < chartdata.length; n++) {
              if (chartdata[n].type === "scatter") {
              } else {
                if (k < chartdata[n].dataPoints.length) {
                  if ("y" in chartdata[n].dataPoints[k]) {
                    ycalc[n] += isNaN(chartdata[n].dataPoints[k].y)
                      ? 0
                      : chartdata[n].dataPoints[k].y;
                  }
                }
              }
            }
  ...
  function toggleDataSeries(e) {
    switch (document.getElementById("legendaction").value) {
      ...
      case "filter_curve":
        ...
        chart.data[activeLine].set(
          "name",
          yold + " filter " + medfil1bin + "/" + averfil1bin
        );
  ...
  document.getElementById("sibionicsFile").onchange = function () {
    const file = this.files[0];
    sibionicsReader.readAsArrayBuffer(file);
  };
  ...
  sibionicsReader.onload = function (e) {
    let blob = e.target.result; // it contains an ArrayBuffer
    sibionicsData = blob;
    processDataIfBothLoaded();
  };
  ...
  function parseBLOB(blob) {
    ...
    parser.parse(blob, function (error, data) {
      if (error) {
        ...
        console.log(
          "file parsing takes: " +
            parseFloat(timeEndParsing - timeStartParsing) +
            " ms"
        );
        //console.log(data);
        prepareFitData(data);
  ...
  function prepareFitData(data) {
    activityLabel =
      new Date(data.sessions[0]["start_time"]).toISOString().slice(0, 10) +
      " " +
      data.sessions[0].sport;
    activityStartTime = new Date(data.records[0].timestamp).getTime();
    activityEndTime = new Date(
      data.records[data.records.length - 1].timestamp
    ).getTime();

    let D = new Date(data.records[0].timestamp);
    let timeoffset = D.getHours() + D.getMinutes() / 60;

    const workbook = XLSX.read(new Uint8Array(sibionicsData), {
      type: "array",
    });
    // 假设数据在第一个工作表中
    ...
    // 筛选在运动时间范围内的血糖数据并转换时间格式
    const bloodSugarData = jsonData
      .slice(1)
      .map((row) => {
        const D = new Date(row[0]);
        return {
          originalTime: D.getTime(),
          timestamp: D.getHours() + D.getMinutes() / 60 - timeoffset,
          bloodSugar: parseFloat(row[1]),
        };
      })
      .filter(
        (data) =>
          data.originalTime >= activityStartTime &&
          data.originalTime <= activityEndTime
      )
      .sort((a, b) => a.timestamp - b.timestamp); // 确保按时间排序

    // 创建一个映射来存储血糖值对应的运动记录索引
    let bloodSugarMapping = new Map();
    let currentBSIndex = 0;

    // 遍历运动记录,为每个血糖值找到最近的未来时间点
    for (let k in data.records) {
      let record = data.records[k];
      D = new Date(record.timestamp);
      let recordTimestamp = D.getHours() + D.getMinutes() / 60 - timeoffset;

      // 检查是否有未匹配的血糖数据
      while (currentBSIndex < bloodSugarData.length) {
        const bs = bloodSugarData[currentBSIndex];

        // 如果当前记录时间大于等于血糖时间,则匹配这条记录
        if (recordTimestamp >= bs.timestamp) {
          bloodSugarMapping.set(parseInt(k), bs.bloodSugar);
          currentBSIndex++;
        } else {
          break;
        }
    ...
    for (let k in data.records) {
      let record = data.records[k];
      D = new Date(record.timestamp);
      data.records[k].timestamp =
        D.getHours() + D.getMinutes() / 60 - timeoffset;
      data.records[k].distance = isNaN(record.distance)
        ? NaN
        : record.distance / 1000;
      paceFlag =
        paceFlag ||
        ("speed" in record && !isNaN(record.speed)) ||
        ("enhanced_speed" in record && !isNaN(record.enhanced_speed));

      // 添加血糖数据
      data.records[k].bloodSugar = bloodSugarMapping.has(parseInt(k))
        ? bloodSugarMapping.get(parseInt(k))
        : NaN;
      if (paceFlag) {
        var speed =
          "speed" in record && !isNaN(record.speed)
        ...
        hreFlag =
          hreFlag ||
          (paceFlag && "heart_rate" in record && !isNaN(record.heart_rate));
        if (hreFlag)
          data.records[k].HRE =
            speed > 0 ? (record.heart_rate * 60) / speed : NaN;
      }
    }
    paceFlag = false;
    hreFlag = false;
    for (let k in data.laps) {
      ...
      paceFlag =
        paceFlag ||
        ("avg_speed" in record && !isNaN(record.avg_speed)) ||
        ("enhanced_avg_speed" in record && !isNaN(record.enhanced_avg_speed));
      if (paceFlag) {
        var speed =
          "avg_speed" in record && !isNaN(record.avg_speed)
            ? record.avg_speed
            : record.enhanced_avg_speed;
        data.laps[k].avg_pace = speed > 0 ? 60 / speed : NaN;
        hreFlag =
          hreFlag ||
          (paceFlag &&
            "avg_heart_rate" in record &&
            !isNaN(record.avg_heart_rate));
        if (hreFlag)
          data.laps[k].avg_HRE =
            speed > 0 ? (record.avg_heart_rate * 60) / speed : NaN;
      }
    }
    ...
    if (existHRV) {
      ...
      for (let k in data.hrv) {
        ...
        for (i = 0; i < data.hrv[k].time.length; i++) {
          ...
          if (dataExist) {
            ...
            const time_RR =
              kRR == 0 ? 0 : data.hrvRR[kRR - 1].time_RR + rr / 3600;
            data.hrvRR.push({
              time_RR: time_RR,
              vRR: rr,
              vCleanRR: rr,
      ...
      for (let k = 1; k < data.hrvRR.length - 1; k++) {
        ...
        if (rr > rrm1 && rr >= rrp1) {
          ...
          //	kold = k;
          //} else {
          data.hrvRR[k].breath_rate_RR =
            1 / 60 / (data.hrvRR[k].time_RR - data.hrvRR[k_old].time_RR);
  ...
  function prepareFitDataMap(fitdata) {
    ...
    if (withGPS) {
      ...
      const label =
        new Date(fitdata.sessions[0]["start_time"]).toISOString().slice(0, 10) +
        " " +
      ...
      Lpopups.push(
        L.popup({
          ...
          .setLatLng(
            trackdata[Math.round((Math.random() * (trackdata.length - 1)) / 2)]
          )
          .setContent(label)
          .openOn(mymap)
      );
      ...
      var faketrack = [
        // fake segment to get popup with averaged
  ...
  document.getElementById("clean").onclick = function (e) {
    // reset zoom
    chart.options.axisX.viewportMinimum = chart.options.axisX.viewportMaximum =
      null;
    // reset y-axis
  ...
  document.getElementById("download").onclick = function (e) {
    ...
    if (document.getElementById("downloadURL").value.indexOf("garmin") > 0)
      ...
      fileUrl =
        "https://connect.garmin.com/proxy/download-service/files/activity/xxxxxxxxxx";
